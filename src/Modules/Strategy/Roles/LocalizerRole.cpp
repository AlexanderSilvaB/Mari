...
void LocalizerRole::Tick(float ellapsedTime, const SensorValues &sensor)
{
    ...
    if(spellBook->strategy.GameState == STATE_PLAYING && onStart)
    {
        ...
        {            
            ...
            if(spellBook->perception.vision.feature.FeatureLostCount < 5)
            {
                searchState = 0;
                wait = 0;
                if(abs(spellBook->perception.vision.feature.FeatureYaw) > Deg2Rad(10.0f))
                {
                    spellBook->motion.Vth = -spellBook->perception.vision.feature.FeatureYaw * 0.5f;
                    spellBook->motion.Vx = 0;
                    //CancelKick();
                }
                else
                {
                    if(abs(spellBook->perception.vision.feature.FeatureYaw) > Deg2Rad(5.0f))
                        spellBook->motion.Vth = -spellBook->perception.vision.feature.FeatureYaw * 0.4f;
                    else
                        spellBook->motion.Vth = 0;

                    if(spellBook->perception.vision.feature.FeatureDistance < 0.45f)
                        spellBook->motion.HeadPitch = Deg2Rad(24.0f);
                    else if(spellBook->perception.vision.feature.FeatureDistance > 0.5f)
                        spellBook->motion.HeadPitch = Deg2Rad(0.0f);
                    if(spellBook->perception.vision.feature.FeatureDistance > 0.25f)
                    {
                        spellBook->motion.Vx = min(spellBook->perception.vision.feature.FeatureDistance * 0.25f, 0.25f);
                        //CancelKick();
                    }
                    else
                    {
