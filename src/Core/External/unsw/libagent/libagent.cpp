...
void Agent::doBattery(float charge, float current, int status)
{
    ...
    {
        SAY("Bateria em " + boost::lexical_cast<string>(battery * 3) +
            " porcento");
    }
    old_battery = battery;
...
void Agent::doButtons(bool chest, bool left, bool right)
{
    ...
    if (chest_up > MAX_CLICK_INTERVAL && chest_presses)
    {
        ...
        if (buttons.pop(2))
        {
            ...
            {
                head_limp = !head_limp;
                SAY(std::string("cabecinha ") +
                    (head_limp ? "mole" : "rígida"));
            }
            else
            {
                if (limp)
        ...
        else if (buttons.pop(3))
        {
            if (left || right)
            {
                SAY("Reiniciando Nao Key"); // yay transliteration
            ...
            else
            {
                if (skipped_frames > MAX_SKIPS)
                {
                    ...
                    if (!fork())
                    {
                        ...
                        ret = system("/usr/bin/killall rinobot");
                        ret = execlp("/home/nao/rinobot", "rinobot",
                                     (char *)NULL);
                    }
                }
                else
                {
                    SAY("Parando o Rinoboto");
                    ret = system("/usr/bin/killall rinobot");
    ...
    { // 3 seconds
        if (left || right)
        {
            ...
            ret = system("/bin/su -c '/usr/bin/killall rinobot'");
...
void Agent::doLEDs(ActionCommand::LED &leds)
{
    ...
    // Battery (Left Ear)
    uint16_t tmp = shared_data->sensors[shared_data->sensors_latest].sensors[Sensors::Battery_Charge] * 10;
      ...
      old_battery_status(0)
{
    ...
    battery_status_pointer = (int *)memory->getDataPtr("Device/SubDeviceList/Battery/Charge/Sensor/Status");
    ...
    for (i = Joints::LShoulderPitch; i < Joints::NUMBER_OF_JOINTS; ++i)
    {
        alias[1][i - 2] = string(Joints::jointNames[i]).append("/Position/Actuator/Value");
    }
    ...
    for (i = Joints::HeadYaw; i <= Joints::HeadPitch; ++i)
    {
        alias[1][i] = string(Joints::jointNames[i]).append("/Position/Actuator/Value");
    }
    dcm->createAlias(alias);
    alias[0] = string("LEDs");
    ...
    for (i = Joints::LShoulderPitch; i < Joints::NUMBER_OF_JOINTS; ++i)
    {
        alias[1][i - 2] = string(Joints::jointNames[i]).append("/Hardness/Actuator/Value");
    }
    dcm->createAlias(alias);
    ...
    for (i = Joints::HeadYaw; i <= Joints::HeadPitch; ++i)
    {
        alias[1][i] = string(Joints::jointNames[i]).append("/Hardness/Actuator/Value");
    }
    dcm->createAlias(alias);
