...
FieldLineDetectionLegacy::FieldLineDetectionLegacy()
{
...
void FieldLineDetectionLegacy::detect(const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out)
{
    unsigned int seed = 42;

    RegionI *region_top = new RegionI((info_middle.full_regions[0]), 2, DENSITY_INCREASE, true, true, true);
    RegionI *region_bot = new RegionI((info_middle.full_regions[1]), 2, DENSITY_INCREASE, true, true, true);

    findFieldFeatures(info_in, info_middle, info_out, *(region_top->getInternalFovea()),
                      *(region_bot->getInternalFovea()), &seed, 2);

    delete region_top;
    delete region_bot;
}

void FieldLineDetectionLegacy::findFieldFeatures(
    ...
    unsigned int *seed, int playerNum)
{
...
*/
    ...
    findFeaturesInFovea(info_in, info_middle, info_out, botFovea, seed);
    ...
    t.restart();
    reset(false);
    //searchForFeatures(info_in, info_middle, info_out, topFovea, seed);
    ...
    for (uint16_t i = 0; i < fieldFeatures.size(); i++)
    {
    ...
    if (numStrongFeatures < 1)
    {
        t.restart();
        reset(false);
        findFeaturesInFovea(info_in, info_middle, info_out, topFovea, seed);
        fieldFeatures.insert(fieldFeatures.end(),
                             fieldLines.begin(),
                             fieldLines.end());
    ...
    //VWong
    //Readded this piece of code that was once removed
    //Parallel lines was removed because if we saw the centre circle as parallel lines, localisation would perform poorly

    // Only look for parallel lines if we haven't seen the centre circle
    /*
   vector<FieldFeatureInfo>::const_iterator it;
   bool seenCircle = false;
   for (it = fieldFeatures.begin(); it != fieldFeatures.end(); ++it) {
      if (it->type == FieldFeatureInfo::fCentreCircle) seenCircle = true;
   }
   if (!seenCircle) findParallelLines(&fieldFeatures);
   */

    // Find Penalty Spot
    //Penalty spot detection was removed because it had a lot of false positives with the black and white ball

    t.restart();
    if (playerNum == 1)
    {
        if (!findPenaltySpot(info_in, info_middle, info_out, botFovea))
        {
            findPenaltySpot(info_in, info_middle, info_out, topFovea);
        }
    }
    ...
    // For localisation,
    // since FIeldLinePointInfo's are found on the ball
    // Remove all of these

    if (playerNum == 1)
    {
        std::vector<FieldFeatureInfo>::iterator features;
        for (features = fieldFeatures.begin(); features != fieldFeatures.end();)
        {
            if (features->type == FieldFeatureInfo::fFieldLinePoint)
            {
                features = fieldFeatures.erase(features);
            }
            else
            {
                features++;
    ...
    info_out.features = fieldFeatures;
}

void FieldLineDetectionLegacy::reset(bool full)
{
...
bool FieldLineDetectionLegacy::findPreviousFeatures(
...
void FieldLineDetectionLegacy::findFeaturesInFovea(
    const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out,
    const Fovea &fovea,
    unsigned int *seed,
    int type)
{
    ...
    findFoveaPoints(info_in, info_middle, info_out, fovea);
...
void FieldLineDetectionLegacy::searchForFeatures(
    const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out,
    const Fovea &f,
    unsigned int *seed)
{
    ...
    for (l = landmarks.begin(); l != landmarks.end(); ++l, ++i)
    {
        ...
        if (count >= 2)
            return; // Stop once we've searched 2 foveas
            ...
            (landmark.y() > 0) && (landmark.y() < TOP_IMAGE_ROWS))
        {
            ...
            boost::shared_ptr<Fovea> fovea(new Fovea(BBox(tl, br), density, true, true, false, true));
            fovea->generate(*combined_frame_, *colour_classifier_);

            foveas.push_back(fovea);

            // Look for features in fovea :)
            findFeaturesInFovea(info_in, info_middle, info_out, *fovea, seed, searchFor);
            fieldFeatures.insert(fieldFeatures.end(),
                                 fieldLines.begin(),
                                 fieldLines.end());
            ...
            ++count;
            if (i < numCircleLandmarks)
                ++count; // Finding circle is expensive, only do it once
   ...
   if (numLines == 1) {
      ...
      if (p1.x() > exclusion && p1.x() < (fovea.getBBox().width()-exclusion) &&
          p1.y() > exclusion && p1.y() < (fovea.getBBox().height()-exclusion)) {
      ...
      if (p2.x() > exclusion && p2.x() < (fovea.getBBox().width()-exclusion) &&
          p2.y() > exclusion && p2.y() < (fovea.getBBox().height()-exclusion)) {
...
void FieldLineDetectionLegacy::findFoveaPoints(
    const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out,
    const Fovea &fovea)
{
    ...
    Point start;
    int fieldTop;
    const int cols = fovea.getBBox().width();
    const int rows = fovea.getBBox().height();
    ...
    for (int i = 0; i < cols; i++)
    {
        ...
        if (fovea.getTop())
        {
            fieldTop = info_out.topStartScanCoords[start.x()];
        }
        else
        {
            fieldTop = info_out.botStartScanCoords[start.x()];
        }
        ...
        {
            if (fovea.getFoveaColour(i, j) == cGREEN)
            {
                lastGreen = j;
            }

            int dx = fovea.getFoveaEdge(i, j).x();
            int dy = fovea.getFoveaEdge(i, j).y();
            ...
            else if (bottomMT.x() != 0)
            {
                ...
                //            Colour c = fovea.getFoveaColour(x,y);
                int pixel = checkPixelColour(Point(x, y), fovea);
                if (pixel == 0)
                {
                    //            if ((c != cWHITE) && (c != cBACKGROUND)){
                    top = bottom;
                    topMT = bottomMT;
                    bottom = Point(0, 0);
                    bottomMT = PointF(0, 0);
                ...
                // FOR BALL DETECTION
                // GRAB TOP AND BOTTOM AND SAVE THEM
                Point imageTop = fovea.mapFoveaToImage(top);
                Point imageBottom = fovea.mapFoveaToImage(bottom);
                Point topRR = info_in.cameraToRR.pose.imageToRobotXY(imageTop);
                Point bottomRR = info_in.cameraToRR.pose.imageToRobotXY(imageBottom);
                ...
                Point p = info_in.cameraToRR.pose.imageToRobotXY(middle);
                ...
                // Save the point for ball detection
                lineEdges.push_back(imageTop);
                lineEdges.push_back(imageBottom);
...
int FieldLineDetectionLegacy::checkPixelColour(Point p, const Fovea &fovea)
{
    bool colour = (fovea.getFoveaColour(p.x(), p.y()) == cWHITE) ||
                  (fovea.getFoveaColour(p.x(), p.y()) == cBACKGROUND);
    if (colour)
        return 10;
    bool top = false;
    bool bottom = false;
    if (p.y() > 1)
    {
        top = (fovea.getFoveaColour(p.x(), p.y() - 1) == cWHITE) ||
              (fovea.getFoveaColour(p.x(), p.y() - 1) == cBACKGROUND);
    }
    if (top)
        return 1;
    if (p.y() < fovea.getBBox().height() - 1)
    {
        bottom = (fovea.getFoveaColour(p.x(), p.y() + 1) == cWHITE) ||
                 (fovea.getFoveaColour(p.x(), p.y() + 1) == cBACKGROUND);
    }
    if (bottom)
        return -1;
...
void FieldLineDetectionLegacy::findIntersections(
        ...
        for (uint16_t j = i + 1; j < (lines.size()); j++)
        {
            ...
            if ((p.x() != 0) && perpendicular(l1, l2))
            {
                ...
                else
                {
                    ...
                    RRCoord r = RRCoord(distance, heading, -angle);
                    p = convertRR->convertToImageXY(p);
                    e1 = convertRR->convertToImageXY(e1);
                    e2 = convertRR->convertToImageXY(e2);
...
void FieldLineDetectionLegacy::circleOrientation(
    vector<FieldFeatureInfo> *features)
{
    ...
    if (fabs(dist) > 200)
        return; // check line is within 20cm of circle centre
...
bool FieldLineDetectionLegacy::closeToEdge(const Fovea &fovea, FieldFeatureInfo f)
{
    Point p;
    ...
    if (p.x() < EXCLUSION_LENGTH)
        return true;
    if (p.y() < EXCLUSION_LENGTH)
        return true;
    if (p.x() > (fovea.getBBox().width() - EXCLUSION_LENGTH))
        return true;
    if (p.y() > (fovea.getBBox().height() - EXCLUSION_LENGTH))
        return true;
    return false;
}

bool FieldLineDetectionLegacy::isBadCorner(LineInfo l1, LineInfo l2)
{
...
float FieldLineDetectionLegacy::findTAngle(Point p, LineInfo l)
{
...
float FieldLineDetectionLegacy::findCAngle(Point p, LineInfo l1, LineInfo l2)
{
...
float FieldLineDetectionLegacy::findGradient(LineInfo l, Point p)
{
...
bool FieldLineDetectionLegacy::possibleT(LineInfo l1,
                                         LineInfo l2)
{
...
Point FieldLineDetectionLegacy::intersect(LineInfo l1, LineInfo l2)
{
    Eigen::Matrix2f A;
    Eigen::Vector2f b, x;
    Point p = Point(0, 0);
...
bool FieldLineDetectionLegacy::perpendicular(LineInfo l1, LineInfo l2)
{
...
bool FieldLineDetectionLegacy::parallelPair(LineInfo l1,
                                            LineInfo l2,
                                            RRCoord *r)
{
    bool parallel = false;
    ...
    if (lenSqr1 < MIN_PARALLEL_LENGTH || lenSqr2 < MIN_PARALLEL_LENGTH)
        return false;
...
void FieldLineDetectionLegacy::findFieldLinesAndCircles(const vector<FieldLinePointInfo> &points,
                                                        vector<FieldFeatureInfo> *lines,
                                                        ...
                                                        unsigned int *seed)
{
    ...
    for (uint16_t i = 0; i < 3; i++)
    {
        ...
        if (RANSAC::findLinesAndCircles(fieldPoints1, CENTER_CIRCLE_DIAMETER / 2, &con, resultLine, resultCircle,
                                        k, e, n, consBuf, seed))
        {
...
void FieldLineDetectionLegacy::findFieldLinesInFovea(const vector<FieldLinePointInfo> &points,
                                                     vector<FieldFeatureInfo> *lines,
                                                     vector< vector<FieldLinePointInfo> > *linePoints,
                                                     const Fovea &fovea,
                                                     unsigned int *seed)
...
void FieldLineDetectionLegacy::checkLine(
...
bool FieldLineDetectionLegacy::isLineTooLong(const Fovea &fovea,
                                             LineInfo l1, LineInfo *l2)
{
    ...
    BresenhamPtr<const Colour> cbp(fovea.getInternalColour(), start, end,
                                   fovea.getBBox().height());
    BresenhamPtr<const Colour>::iterator cp = cbp.begin();

    int it_count = 0;
    int green_count = 0;
    while (fovea.getBBox().validIndex(cp.point()))
    {
        ++cp;
        it_count++;
        if (it_count > 6)
        ...
        if (*cp == cGREEN)
        {
            ...
            if (green_count < 2)
                continue;
            while (fovea.getBBox().validIndex(cp.point()) && (*cp != cWHITE))
            {
                ++cp;
            }
            if (*cp == cWHITE)
    ...
    cbp = BresenhamPtr<const Colour>(fovea.getInternalColour(), end, start, fovea.getBBox().height());
    cp = cbp.begin();
...
void FieldLineDetectionLegacy::findParallelLines(
...
bool FieldLineDetectionLegacy::findPenaltySpot(const VisionInfoIn &info_in, const VisionInfoMiddle &info_middle, VisionInfoOut &info_out,
                                               const Fovea &fovea)
{
    /*
   VictorW:
   XHistogram xhistogram(fovea.getBBox().width());// = fovea.xhistogram;
   YHistogram yhistogram(fovea.getBBox().height());// = fovea.yhistogram;
   ...
   for (it = horz.begin(); it != horz.end(); ++it) {
      for (jt = vert.begin(); jt != vert.end(); ++jt) {
         ...
         for (int i = hStart; i < hEnd; ++i) {
            if (fovea.getFoveaColour(i, vMid) != cWHITE) hStart++;
            else break;
         }
         for (int i = hEnd-1; i >= hStart; --i) {
            if (fovea.getFoveaColour(i, vMid) != cWHITE) hEnd--;
            else break;
         }
         for (int i = vStart; i < vEnd; ++i) {
            if (fovea.getFoveaColour(hMid, i) != cWHITE) vStart++;
            else break;
         }
         for (int i = vEnd-1; i >= vStart; --i) {
            if (fovea.getFoveaColour(hMid, i) != cWHITE) vEnd--;
            else break;
         }
         ...
         if ((tl.x() < 0) || (br.x() > fovea.getBBox().width()-1) ||
             (tl.y() < 0) || (br.y() > fovea.getBBox().height()-1)) {
         ...
         for (int i = tl.x(); i < br.x(); ++i) {
            if (fovea.getFoveaColour(i, tl.y()) == cGREEN) ++top;
            if (fovea.getFoveaColour(i, br.y()) == cGREEN) ++bottom;
            if (fovea.getFoveaColour(i, centre.y()) == cWHITE ||
                fovea.getFoveaColour(i, centre.y()) == cBACKGROUND) ++horizontal;
         }
         for (int i = tl.y(); i < br.y(); ++i) {
            if (fovea.getFoveaColour(tl.x(), i) == cGREEN) ++left;
            if (fovea.getFoveaColour(br.x(), i) == cGREEN) ++right;
            if (fovea.getFoveaColour(centre.x(), i) == cWHITE ||
                fovea.getFoveaColour(centre.x(), i) == cBACKGROUND) ++vertical;
         }
...
/*
void FieldLineDetectionLegacy::makeWhiteHistsBelowFieldEdge(
      const VisionInfoIn& info_in, const VisionInfoMiddle& info_middle, VisionInfoOut& info_out,
      const Fovea &fovea,
      ...
      Histogram<int, cNUM_COLOURS> &yhist)
{
   ...
   for (y = 0; y < fovea.getBBox().height(); ++ y) {
      yhist._counts[y][cWHITE] = 0;
   }
   for (x = 0; x < fovea.getBBox().width(); ++ x) {
   ...
   for (x = 0; x < fovea.getBBox().width(); ++ x) {
      ...
      end = fovea.mapFoveaToImage(Point(x, fovea.getBBox().height()));
      if (fovea.top) {
      ...
      end.y() = std::min(end.y(), fovea.getBBox().height());
      ...
      saliencyPixel = fovea._colour + start.x() * fovea.getBBox().height() + start.y();
      saliencyEnd   = fovea._colour + end.x()   * fovea.getBBox().height() + end.y();
...
void FieldLineDetectionLegacy::findCornerEndpoints(LineInfo l1, LineInfo l2,
                                                   Point &e1, Point &e2)
{
