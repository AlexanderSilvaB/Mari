...
void LocalisationAdapter::tick() {
   ...
   bool doingBallLineUp = false; //readFrom(behaviour, behaviourSharedData).doingBallLineUp;
   L->setLineUpMode(doingBallLineUp);

   bool isInReadyMode = false; //readFrom(behaviour, behaviourSharedData).isInReadyMode;
   L->setReadyMode(isInReadyMode);

   //if (readFrom(gameController, data).state == STATE_INITIAL && readFrom(behaviour, skill) == "GameController") {
   if (readFrom(gameController, data).state == STATE_INITIAL) {
   ...
   // Make sure we set ourselves to manual placement if penalised before we
   // enter set.
   if(isPenalised && readFrom(gameController, data).state != STATE_SET &&
                 readFrom(gameController, data).state != STATE_PENALISED_IN_SET)
       penalised_before_set = true;
   if(!isPenalised)
       penalised_before_set = false;
   ...
   if ((readFrom(gameController, data).state == STATE_SET ||
        readFrom(gameController, data).state == STATE_PENALISED_IN_SET) &&
                       ((isPickedUpHysteresis == 10) || penalised_before_set)) {
      L->resetToManualPlacement(readFrom(gameController, data).kickOffTeam);
   ...
   //} else if (canLocaliseInState(readFrom(gameController, data).state, readFrom(behaviour, skill))) {
      } else if (canLocaliseInState(readFrom(gameController, data).state, "GameController")) {
   ...
   if (canLocaliseInState(readFrom(gameController, data).state, "GameController") &&
       !isInPenaltyShootout) {
