...
public:
   ...
   enum LocalisationConstant {
      ...
      // If a mode is likely (low enough variance and combines with another mode), then we increase 
      // the weight of this mode by LIKELY_MODE_WEIGHT_ADJUSTMENT 
      LIKELY_MODE_WEIGHT_ADJUSTMENT,
      ...
      // If something is more than this amount off the field, we deem it to be off the field
      ACCEPTABLE_OFF_FIELD_ERROR_MARGIN
   };
