...
static float distLineToBoundary(const LineInfo line, const FieldBoundaryInfo boundary); 
...
int ICP::localise(  const AbsCoord &robotPos,
   ...
   preprocessFeatures(fieldFeatures, posts, fieldBoundaries);
...
void preprocessFeatures( const std::vector<FieldFeatureInfo> &fieldFeatures,
      ...
      switch (feature->type) {
         ...
         case FieldFeatureInfo::fParallelLines:
            {
               ...
               std::vector<FieldBoundaryInfo>::const_iterator edge = fieldBoundaries.begin();
               for(; edge!=fieldBoundaries.end(); ++edge){
                  float dist1 = distLineToBoundary(pl.l1, *edge);
                  float dist2 = distLineToBoundary(pl.l2, *edge);
         ...
         case FieldFeatureInfo::fLine:
            if ( !feature->lineUsed){ // if not used in an intersection
               ...
               std::vector<FieldBoundaryInfo>::const_iterator edge = fieldBoundaries.begin();
               for(; edge!=fieldBoundaries.end(); ++edge){
                  float dist = distLineToBoundary(fieldLines.back(), *edge); 
         ...
         case FieldFeatureInfo::fCorner:
            {            
               ...
               for(; edge!=fieldBoundaries.end(); ++edge){
                  ...
                  float dist_left = distLineToBoundary(left_arm , *edge);
                  float dist_right = distLineToBoundary(right_arm , *edge);        
   ...
   if (totalN == 0){
      ...
      for(; edge!=fieldBoundaries.end(); ++edge){
         LineInfo line;
         line.p1 = edge->rrBoundary.p1;
         line.p2 = edge->rrBoundary.p2;
...
float distLineToBoundary(const LineInfo line, const FieldBoundaryInfo boundary){

   PointF lineVector = (line.p2 - line.p1).cast<float>();
   lineVector /= hypot(lineVector.x(), lineVector.y());               
   
   PointF edgeVector = (boundary.rrBoundary.p1 - boundary.rrBoundary.p2).cast<float>();
   edgeVector /= hypot(edgeVector.x(), edgeVector.y());   

   // Check if boundary is close to parallel                  
   float dot = lineVector.dot(edgeVector);
   if ( fabs(dot) > cos(PARALLEL_THRESH) ){
      LineInfo edgeline;
      edgeline.p1 = boundary.rrBoundary.p1;
      edgeline.p2 = boundary.rrBoundary.p2;
