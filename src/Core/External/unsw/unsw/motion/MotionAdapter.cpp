...
void MotionAdapter::tick()
{
    ...
    if (steps == 0)
    {
        request.stiffen = ActionCommand::STIFFEN;
        steps++;
    }
    else if (steps == 1)
    {
        request.body = ActionCommand::Body(ActionCommand::Body::STAND);
        steps++;
    }
    else if (steps >= 500)
    {
        request.body = ActionCommand::Body(ActionCommand::Body::WALK, 300, 0, 0);
        //request.body = ActionCommand::Body(ActionCommand::Body::KICK);
        //request.body = ActionCommand::Body(ActionCommand::Body::MOTION_CALIBRATE);
        steps++;
    }
    ...
    //request = readFrom(behaviour, request[behaviourReadBuf]).actions;
