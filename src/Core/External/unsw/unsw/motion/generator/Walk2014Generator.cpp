...
#define KICK_HIP_HEIGHT_REDUCTION_PERCENT -0.095238
...
const float BASE_WALK_PERIOD = .25; //.25;                 // seconds to walk one step, ie 1/2 walk cycle
const float WALK_HIP_HEIGHT = .21; // Walk hip height - seems to work from .2 to .235
...
const float KICK_CROUCH_PERIOD = 0.5;
...
JointValues Walk2014Generator::makeJoints(ActionCommand::All* request, Odometry* odometry, const SensorValues &sensors, BodyModel &bodyModel, float ballX, float ballY) {
    ...
    if (t == 0 && walk2014Option != STEP) {
        ...
        // Add a speed cap here if name is bb8 (sorry this is for nagoya comp, no time to improve)
    ...
    if (request->body.actionType != ActionCommand::Body::KICK && kickT > 0) {
        ...
        if (lastKickTime < 4 * T) {
        ...
        } else if (abs(hiph - (WALK_HIP_HEIGHT-KICK_HIP_HEIGHT_REDUCTION_PERCENT*WALK_HIP_HEIGHT)) < .0001) { // make sure we retain the designated walking height
            ...
            if (walk2014Option != KICK_CROUCH) { // robot starts crouching to walking height
                hiph0 = hiph;
                timer = 0;
            }
            walk2014Option = KICK_CROUCH;                       // continue crouching
        }
    ...
    } else if (walk2014Option == KICK_CROUCH){
        forward = left = turn = 0;
        stiffness = 1;
        hiph = hiph0 + (WALK_HIP_HEIGHT - (KICK_HIP_HEIGHT_REDUCTION_PERCENT*WALK_HIP_HEIGHT) - hiph0) * parabolicStep(timer, KICK_CROUCH_PERIOD, 0);
        comOffset = COM_OFFSET * parabolicStep(timer, CROUCH_STAND_PERIOD, 0); // move comOffset to 0.01 meters when walking
        t = nextFootSwitchT = 0;
        timer += dt;                                        // inc. option timer
    ...
    if (walk2014Option == WALK and nextFootSwitchT > 0) {
        ...
        if (active.foot == ActionCommand::Body::LEFT) {
            makeForwardKickJoints(kickLean, KICK_STEP_HEIGHT, foothL, forwardL, leftL, kneePitchL, shoulderRollL, anklePitchL, ballY, request);
            leftR = leftL * 0.1; // Balance slightly more over support foot if need be
        } else { // with added adjustments for right side
            makeForwardKickJoints(-kickLean + 1.5, KICK_STEP_HEIGHT + 0.005, foothR, forwardR, leftR, kneePitchR, shoulderRollR, anklePitchR, ballY, request);
...
void Walk2014Generator::makeForwardKickJoints(float kickingLean, float kickStepH, float &footh, float &forwardDist, float &side, float &kneePitch,
        float &shoulderRoll, float &anklePitch, float &ballY, ActionCommand::All* request) {
    ...
    if (kickT < backPhase) {
        ...
        forwardDist = interpolateSmooth(kickBackAmp, kickAmp, kickT - backPhase, kickPhase);
        side = sideAmp;
        shoulderRoll = shoulderRollAmp;
        kneePitch = interpolateSmooth(kneePitchAmp * 0.9, -kneePitchAmp, kickT - backPhase, kickPhase);
        if (kickT >= backPhase) {
            anklePitch = interpolateSmooth(anklePitchRetracted, anklePitchAmp, kickT - backPhase, kickPhase);
        }
        // Hold...
        ...
        rock = lastRock * sin((endT + shiftEndPeriod / 4) / shiftEndPeriod * 2 * M_PI);
        footh = lastFooth * sin((endT + shiftEndPeriod / 4) / shiftEndPeriod * 2 * M_PI);
    } else {
        kickT = 0;
        rock = 0;
        footh = 0;
        walk2014Option = CROUCH;
        walkState = NOT_WALKING;
        request->body.actionType = ActionCommand::Body::WALK;
        lastKickTime = 0;
    }
